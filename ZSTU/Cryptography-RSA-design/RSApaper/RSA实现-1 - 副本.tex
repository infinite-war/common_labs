...
\documentclass[UTF8,a4paper,10.5pt,twocolumn]{ctexart}
\usepackage[left=1.80cm, right=1.20cm, top=2.80cm, bottom=3.90cm]{geometry} %页边距
...
\renewcommand{\abstractname}{\textbf{\large {}}} %更改摘要二字的样式
...
\begin{abstract}
\noindent
\textbf{\heiti 摘要:} RSA密码算法是目前在信息安全领域广为使用的公钥加密方法。该算法涉及欧几里得、欧拉函数、乘法逆元等基本数论知识，易于实现。基于Java的BigInterger类型，设计并实现了RSA公钥密码系统中的密钥生成、加密算法、解密算法。其中，通过将扩展欧几里得算法转换成递推形式降低了算法的空间复杂度。\\
\noindent
\textbf{\heiti 关键字:} RSA  \space\space Java \space\space BigInteger
\end{abstract}
...
\noindent
\textbf{Keywords:} RSA \space\space  Java \space\space   BigInteger
~\\	
~\\	
\end{abstract}
\end{@twocolumnfalse}
...
\section{实验结果}
	...
	%\begin{tabular}{p{0.18\textwidth}<{\centering}|p{0.18\textwidth}<{\centering}}%
   %     \hline
    %        参数& 值 \\
   %     \hline
    %        $p$ &23087 \\
