...
//  move[i][j]����   ��״̬i ��ת�� ״̬j   ����Ҫ���ַ�����
// ����  ״̬A��״̬B   ����ͨ��{'x','y','z'}��һ�����е��ַ�ʵ����ת
...
void createNFA()
{
	...
	while(N--)
	{
		...
		else
		{
			...
			//else
			//��'Y'������̬
			move[ch][str[0]].set[move[ch][str[0]].len++]='Y';
		}
	}
}
...
bool DFA(char str[])
{
	...
	for(int i=0;i<strlen(str);++i )
	{
		...
		if(now_state==-1) return false;
	}
	if(is_final[now_state]==true) return true;
	return false;
}
...
int main()
{
	...
	output.open("F:\\����ԭ���γ���ƴ��\\output.txt");
	scan();
	fclose(file_source);
	output.close();
	return 0;
